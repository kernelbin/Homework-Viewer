...
void __inline AlphaOnlygaussianHorizontal(unsigned char* bufferPerLine, unsigned char* lpRowInitial, unsigned char* lpColumn, int width, int height, int Channels, int Nwidth, float a0a1, float a2a3, float b1b2, float  cprev, float cnext)
{
	int HeightStep = Channels * height;
	int WidthSubOne = width - 1;
...
void __inline AlphaOnlygaussianVertical(unsigned char* bufferPerLine, unsigned char* lpRowInitial, unsigned char* lpColInitial, int height, int width, int Channels, float a0a1, float a2a3, float b1b2, float  cprev, float  cnext)
{
...
void  AlphaOnlyGaussianBlurFilter(unsigned char* input, unsigned char* output, int Width, int Height, int Stride, float GaussianSigma) {
	...
	for (register int y = 0; y < Height; ++y) {
		unsigned char* lpRowInitial = input + Stride * y;
		unsigned char* lpColInitial = tempData + y * Channels;
		AlphaOnlygaussianHorizontal(bufferPerLine, lpRowInitial, lpColInitial, Width, Height, Channels, Width, a0a1, a2a3, b1b2, cprev, cnext);
	}
	int HeightStep = Height * Channels;
	for (register int x = 0; x < Width; ++x) {
		unsigned char* lpColInitial = output + x * Channels;
		unsigned char* lpRowInitial = tempData + HeightStep * x;
		AlphaOnlygaussianVertical(bufferPerLine, lpRowInitial, lpColInitial, Height, Width, Channels, a0a1, a2a3, b1b2, cprev, cnext);
	}
